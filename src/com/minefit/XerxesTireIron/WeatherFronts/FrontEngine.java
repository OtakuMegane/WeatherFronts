...
public class FrontEngine implements Listener {
    ...
    public void randomStrike(World world, String simulator, Map<String, Integer> frontMap, Chunk[] validChunks) {
        ...
        if ((!test.biomeIsDry(highLoc) && !test.biomeIsCold(highLoc)) || (test.biomeIsDry(highLoc) && lightningDry)
                || (test.biomeIsCold(highLoc) && lightningCold)) {
    ...
    public void moveFront(World world, String simulator, String frontName) {
        ...
        if (frontMap.get("velocity-x") != 0 && frontMap.get("velocity-z") != 0) {
            ...
            if (frontX > simCenterX + simRange || frontX < simCenterX - simRange || frontZ > simCenterZ + simRange
                    || frontZ < simCenterZ - simRange) {
    ...
    public void hydrateFarmland() {
        for (Block block : farmland.keySet()) {
            if (test.worldIsEnabled(block.getWorld()) && block.getType() == Material.SOIL
                    && test.locationIsInRain(block.getLocation())) {
    ...
    public Boolean blockCanHaveSnow(Location location) {
        ...
        if ((block2Type.isOccluding() || block2Type == Material.TNT || block2Type == Material.REDSTONE_BLOCK
                ...
                && (block2Type != Material.ENDER_PORTAL_FRAME && block2Type != Material.MOB_SPAWNER && block2Type != Material.ICE
                        && block2Type != Material.PACKED_ICE && block.getLightFromBlocks() < 10)) {
            return true;
        } else {
            return false;
        }
