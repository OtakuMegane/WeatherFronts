...
public class FireHandler implements Listener {
    ...
    public void extinguishFire() {
        for (Block block : fireBlocks.keySet()) {
            ...
            if (!test.locationIsLoaded(location) || block.getType() != Material.FIRE
    ...
    public void onBlockIgnite(BlockIgniteEvent event) {
        ...
        if(!Configuration.main_config.getBoolean(simConfig + "lightning-fire-in-spawn-chunk", false)
                && event.getCause() == IgniteCause.LIGHTNING && test.locationInSpawnChunk(location))
        {
            event.setCancelled(true);
            return;
        }

        if (test.worldIsEnabled(world) && test.locationIsInFront(location)
                && (test.locationIsInRain(location) || test.adjacentBlockExposed(block))) {
    ...
    public void fulgurite(World world, Block block) {
        ...
        if(!Configuration.main_config.getBoolean(simConfig + "fulgurite-in-spawn-chunk", false)
                && test.locationInSpawnChunk(block.getLocation()))
        {
            return;
        }
